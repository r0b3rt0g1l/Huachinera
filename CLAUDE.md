# CLAUDE.md — Portal Municipal de Huachinera

Memoria de trabajo para Claude Code: contexto, reglas duras y estado actual del
proyecto. Léelo antes de tocar el repo.

## Qué es
Sitio oficial del **H. Ayuntamiento de Huachinera, Sonora** (administración 2024-2027):
...
## Color institucional (PROVISIONAL)
La paleta de Huachinera es **emerald/cyan PROVISIONAL** (pendiente paleta definitiva). Los nombres
`--color-guinda*`/`--color-dorado*` se heredan de la plantilla; los valores reales:
- `--color-guinda: #059669` (esmeralda), `-deep: #064E3B`, `-soft: #10B981`; cian
  `--color-dorado: #0891B2`, `-soft: #06B6D4`.
- Definida en `app/globals.css` (`@theme`) y espejada en `municipalConfig.paleta`.
- Tipografía: **Inter** (sin serifas), de la plantilla base. <<PENDIENTE: paleta definitiva>>.
...
- ⏳ **Pendientes externos** (no de código): fotos reales del municipio; dominio
  **huachinera.gob.mx** (si aplica; hoy se sirve en huachinera-gobierno.vercel.app);
