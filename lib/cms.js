...
// GUARD 1: el slug es obligatorio en build time y se hornea en el bundle.
// Falla ruidoso (no fallback silencioso) para evitar el bug de jun-2026 en el
// que un slug ausente/genérico hacía leer un tenant inexistente y todo el
// contenido caía al fallback estático sin avisar.
const MUNICIPIO_SLUG = process.env.NEXT_PUBLIC_MUNICIPIO_SLUG;
if (!MUNICIPIO_SLUG || MUNICIPIO_SLUG === "municipio") {
  throw new Error(
    `[cms] NEXT_PUBLIC_MUNICIPIO_SLUG inválido o ausente (valor: ${JSON.stringify(
      MUNICIPIO_SLUG,
    )}). Debe definirse en build time — en Vercel: Settings → Environment ` +
      `Variables (p. ej. "huachinera"); en local: copia .env.example a .env.local ` +
      `o exporta NEXT_PUBLIC_MUNICIPIO_SLUG=huachinera antes de \`npm run build\`. ` +
      `Sin un slug válido el portal leería un tenant inexistente y mostraría el ` +
      `fallback estático.`,
  );
}

// GUARD 2: en build de Vercel, NEXT_PUBLIC_API_URL es obligatoria. Sin ella el
// portal nunca llama al backend y todo el contenido cae al fallback estático en
// silencio. En local puede faltar (se usan los fallbacks a propósito).
if (process.env.VERCEL && !process.env.NEXT_PUBLIC_API_URL) {
  throw new Error(
    "[cms] NEXT_PUBLIC_API_URL ausente en build de Vercel. Defínela en Settings → " +
      "Environment Variables (https://cmsmunicipal.onrender.com) y redeploya con " +
      "BUILD FRESCO (un redeploy de build viejo no la hornea).",
  );
}
...
export async function getHeroSlides() {
  ...
  try {
    ...
    // Guard M3: el slide debe tener imagen usable; sin imagenUrl no se pasa al
    // <Image> del HeroCarousel (evita <Image src={null}> que rompe el render).
    const activeSlides = data.filter(
      (item) => item && item.activo === true && item.imagenUrl,
    );
...
export async function getImagenesFromCMS() {
  ...
  try {
    ...
    // Guard M3: solo imágenes con url usable (evita <Image src={null}> en GaleriaGrid).
    const conImagen = data.filter((item) => item && item.url);
...
function mapFuncionarioCMSToPortal(item) {
  return {
    ...
    bio: item.bio || null,
    // PRIVACIDAD (política Northa): el portal NUNCA publica contacto directo de
    // funcionarios — cero mailto:/tel: — aunque el CMS los traiga. Se fuerzan a
    // null aquí para que PresidenteCard/RegidorCard/PersonDetailModal jamás los
    // rendericen. El contacto del ayuntamiento va solo por el formulario.
    email: null,
    telefono: null,
